...
# 定义操作符类：添加体型
class RT_OT_AddBodyType(Operator):
    bl_idname = "rt.add_body_type"
    bl_label = "添加体型"
    ...
    def execute(self, context):
        scene = context.scene
        new_body_type = scene.rt_new_body_type_input.strip()

        if not new_body_type:
            self.report({'WARNING'}, "新体型名称不能为空")
            return {'CANCELLED'}

        # 检查是否已存在 (忽略大小写和前后空格)
        existing_types_lower = [item[0].lower() for item in scene.rt_character_body_type]
        if new_body_type.lower() in existing_types_lower:
            self.report({'INFO'}, f"体型 '{new_body_type}' 已存在")
            scene.rt_new_body_type_input = "" # 清空输入框
            return {'FINISHED'}

        current_custom_types = scene.rt_custom_body_types
        if current_custom_types:
            scene.rt_custom_body_types = f"{current_custom_types},{new_body_type}"
        else:
            scene.rt_custom_body_types = new_body_type
        
        self.report({'INFO'}, f"成功添加体型: {new_body_type}")
        scene.rt_new_body_type_input = "" # 清空输入框
        # 强制UI刷新，以便EnumProperty能够获取到最新的items
        # 这通常通过更新一个无关的属性或重新注册属性来实现，但更简单的方式是让用户手动刷新或Blender下次UI更新时自动处理
        # bpy.ops.wm.redraw_timer(type='DRAW_WIN_SWAP', iterations=1) # 尝试强制重绘，可能不总有效
        return {'FINISHED'}
